...
origin_name = st.sidebar.selectbox("From (Origin)", city_names, index=city_names.index("Delhi") if "Delhi" in city_names else 0)
dest_name = st.sidebar.selectbox("To (Destination)", city_names, index=city_names.index("Mumbai") if "Mumbai" in city_names else 1)
...
with st.spinner("Fetching OSM corridor and computing road routes..."):
    ...
    road_routes = compute_candidate_routes(
        (G, source),
        origin,
        dest,
        k=k_routes,
...
with road_tab:
    ...
    if road_routes:
        ...
        st.write("**Road routes (k={})**".format(k_routes))
        st.dataframe(
            df_road[
                [
                    "route_id",
...
with train_tab:
    ...
    if len(df_tr) == 0:
        st.info("Add rows to data/trains.csv for {}→{} to see train line‑ups.".format(origin_name, dest_name))
    else:
        ...
        if rec_idx_tr >= 0:
            st.success("Recommended train ({}): {}".format(objective, df_tr_sum.loc[rec_idx_tr, "label"]))
...
with air_tab:
    ...
    if len(df_fl) == 0:
        st.info("Add rows to data/flights.csv for {}→{} to see flight line‑ups.".format(origin_name, dest_name))
    else:
        dist_km = haversine_km(origin[0], origin[1], dest[0], dest[1])
        ...
        if rec_idx_fl >= 0:
            st.success("Recommended flight ({}): {}".format(objective, df_fl_sum.loc[rec_idx_fl, "label"]))
...
with compare_tab:
    ...
    if road_routes and len(df_road) > 0:
        rows.append(
            {
                "mode": "Road",
                "time_min": float(df_road["total_time_min"].min()),
                "cost_inr": float(df_road["total_cost_inr"].min()),
                "emissions_kg": float(df_road["total_emissions_kg"].min()),
            }
        )
    df_tr = load_trains(origin_name, dest_name)
    if len(df_tr) > 0:
        ...
        rows.append(
            {
                "mode": "Train",
                "time_min": float(df_tr_sum["total_time_min"].min()),
                "cost_inr": float(df_tr_sum["total_cost_inr"].min()),
                "emissions_kg": float(df_tr_sum["emissions_kg"].min()),
            }
        )
    df_fl = load_flights(origin_name, dest_name)
    if len(df_fl) > 0:
        ...
        rows.append(
            {
                "mode": "Air",
                "time_min": float(df_fl_sum["total_time_min"].min()),
                "cost_inr": float(df_fl_sum["total_cost_inr"].min()),
                "emissions_kg": float(df_fl_sum["emissions_kg"].min()),
            }
        )

    if rows:
        df_modes = pd.DataFrame(rows)
        st.dataframe(df_modes)
        ...
        st.success("Overall recommended mode ({}): {}".format(objective, df_modes.loc[best_idx, "mode"]))
    else:
